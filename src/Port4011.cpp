...
nts::Port4011::Port4011(std::string const value)
{
    ...
    for (size_t i = 0; i != 14; i++)
        _pin[i] = nullptr;
    use_input = std::bind(&nts::Port4011::doInput, this, std::placeholders::_1);
    use_output = std::bind(&nts::Port4011::doOutput, this, std::placeholders::_1);
...
nts::Tristate nts::Port4011::compute(std::size_t pin)
{
    return _execPin[pin](pin);
}

void nts::Port4011::setLink(std::size_t pin, nts::IComponent &other, std::size_t otherPin)
{
...
void nts::Port4011::dump() const
{
    std::cout << "##########PORT 4011###########" << std::endl;
    for (size_t i = 0; i != 14; i++) {
        std::cout << "\tNUNBER " << i + 1 << " ";
        if (_pin[i] == nullptr)
...
nts::Tristate nts::Port4011::doInput(size_t pin)
{
...
nts::Tristate nts::Port4011::doOutput(size_t pin)
{
    size_t real_pin = pin - 1;
    size_t one;
    size_t two;
...
nts::Tristate nts::Port4011::nandGate(nts::Tristate one, nts::Tristate two)
{
