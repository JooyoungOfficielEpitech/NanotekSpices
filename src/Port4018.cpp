...
nts::Port4018::Port4018(std::string const value)
{
    ...
    for (size_t i = 0; i != 14; i++)
        _pin[i] = nullptr;
    use_input = std::bind(&nts::Port4018::doInput, this, std::placeholders::_1);
    use_output = std::bind(&nts::Port4018::doOutput, this, std::placeholders::_1);
...
nts::Tristate nts::Port4018::compute(std::size_t pin)
{
    return _execPin[pin](pin);
}

void nts::Port4018::setLink(std::size_t pin, nts::IComponent &other, std::size_t otherPin)
{
...
void nts::Port4018::dump() const
{
    std::cout << "##########PORT 4018###########" << std::endl;
    for (size_t i = 0; i != 14; i++) {
        std::cout << "\tNUNBER " << i + 1 << " ";
        if (_pin[i] == nullptr)
...
nts::Tristate nts::Port4018::doInput(size_t pin)
{
...
nts::Tristate nts::Port4018::doOutput(size_t pin)
{
    size_t real_pin = pin - 1;
    size_t one;
    size_t two;
    ...
    return andGate(_pin[one]->compute(_link.at(one)), _pin[two]->compute(_link.at(two)));
}


nts::Tristate nts::Port4018::andGate(nts::Tristate one, nts::Tristate two)
{
