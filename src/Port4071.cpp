...
nts::Port4071::Port4071(std::string const value)
{
    ...
    for (size_t i = 0; i != 14; i++)
        _pin[i] = nullptr;
    use_input = std::bind(&nts::Port4071::doInput, this, std::placeholders::_1);
    use_output = std::bind(&nts::Port4071::doOutput, this, std::placeholders::_1);
...
nts::Tristate nts::Port4071::compute(std::size_t pin)
{
    return _execPin[pin](pin);
}

void nts::Port4071::setLink(std::size_t pin, nts::IComponent &other, std::size_t otherPin)
{
//    std::cout << "Setlink for 4071\n";
...
void nts::Port4071::dump() const
{
    std::cout << "##########PORT 4071###########" << std::endl;
    for (size_t i = 0; i != 14; i++) {
        std::cout << "\tNUNBER " << i + 1 << " ";
        if (_pin[i] == nullptr)
...
nts::Tristate nts::Port4071::doInput(size_t pin)
{
...
nts::Tristate nts::Port4071::doOutput(size_t pin)
{
    size_t real_pin = pin - 1;
    size_t one;
    size_t two;
    ...
    return orGate(_pin[one]->compute(_link.at(one)), _pin[two]->compute(_link.at(two)));
}


nts::Tristate nts::Port4071::orGate(nts::Tristate one, nts::Tristate two)
{
